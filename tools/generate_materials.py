...
for p in ROOT.joinpath('tools').glob('fsd2_bundle_*.b64'): p.unlink()
...
def diagram(path,title,mode):
    im=Image.new('RGB',(1280,720),'white'); d=ImageDraw.Draw(im); d.text((45,35),title,font=font(38),fill='black')
    boxes=[('1. Input','Browser / terminal'),('2. Process','Code runs here'),('3. Result','Screen / API output')]; xs=[60,455,850]
    if mode=='workflow': boxes=[('Client','Request'),('Framework','Route / component'),('Data','Database / state'),('Output','Response / screen')]; xs=[35,335,650,965]
    for x,(a,b) in zip(xs,boxes):
        d.rounded_rectangle((x,270,x+245,450),18,outline='black',width=3); d.text((x+18,305),a,font=font(25),fill='black'); d.text((x+18,355),b,font=font(21),fill='black')
    for i in range(len(xs)-1): d.line((xs[i]+245,360,xs[i+1],360),fill='black',width=5)
    d.text((50,560),'FSD2 learning aid: follow the request/value from left to right.',font=font(22),fill='black'); im.save(path)

def add_title(slide,title,subtitle=''):
    slide.shapes.title.text=title; slide.shapes.title.text_frame.paragraphs[0].font.size=Pt(28)
    if subtitle: slide.placeholders[1].text=subtitle; slide.placeholders[1].text_frame.paragraphs[0].font.size=Pt(18)

def add_bullets(prs,title,items):
    s=prs.slides.add_slide(prs.slide_layouts[1]); add_title(s,title); tf=s.placeholders[1].text_frame; tf.clear()
    for i,x in enumerate(items):
        p=tf.paragraphs[0] if i==0 else tf.add_paragraph(); p.text=x; p.font.size=Pt(18); p.space_after=Pt(8)
    return s

def build(folder):
    title,tasks=title_tasks(folder/'README.md'); n=folder.name[:2]; slug=re.sub(r'[^A-Za-z0-9]+','_',title).strip('_')
    ...
    if code_files:
        snippet=text(code_files[0]).splitlines()[:22]; add_bullets(prs,'Code pathway',[f'{code_files[0].name}:{i} {line}' for i,line in enumerate(snippet,1)])
...
for n,title,slug in rows: idx += [f'## {n}. {title}',f'- [PPT](PPTs/{n}_{slug}.pptx)',f'- [Runtime PDF](PDFs/{n}_{slug}_Runtime.pdf)',f'- [Workflow image](docs/assets/{n}_workflow.png)',f'- [Runtime image](docs/assets/{n}_runtime.png)','']
(ROOT/'MATERIALS_INDEX.md').write_text('\n'.join(idx),encoding='utf-8'); print(f'Generated materials for {len(rows)} experiments.')
